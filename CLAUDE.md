...
| **"포트폴리오 건강도 리뷰 하자"** | `IDEA.md` 기준 — 포트폴리오 대장 로드 후 STEP 1·2·4·5·6 실행, 건강도 리포트 저장 |

> **"제네시스 하자"** 명령 시 반드시 `workflows/05_발굴-실행-워크플로우.md` 를 읽고 STEP 1~6 순서대로 실행한다.

> **"포트폴리오 건강도 리뷰 하자"** 명령 시 반드시 `IDEA.md` 를 읽고 아래 순서대로 실행한다:
> 1. `C:\Users\hwang\OneDrive\바탕 화면\포트폴리오분석\포트폴리오관리\포트폴리오-대장.md` 로드 → 보유 종목 코드·분류·섹터구분 추출
> 2. STEP 1 시황 판단 → STEP 2 대세섹터 확정 → STEP 4 전종목 스코어링 → STEP 5 4버킷 태깅 → STEP 6 리포트 생성
> 3. 리포트 저장: `C:\Users\hwang\OneDrive\바탕 화면\포트폴리오분석\포트폴리오관리\reviews\YYYYMMDD_포트폴리오건강도.md`
